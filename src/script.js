...
function renderHolidays() {
    ...
    state.holidays.forEach(({ date, summary }) => {
        ...
        div.innerHTML = `${formatDate(date)} ${summary}`;
        refs.holidayContainer.appendChild(div);
    });
}

// ─── Date Calculation (DEPENDENT TASKS) - FIXED VERSION ──────────────────────
function calculateDatesForAllTasks() {
    if (!state.deadline || state.tasks.length === 0) return;

    // FIXED: Allow work ON the deadline if it's a working day
...
// NEW: Find the working day before a given date (for task dependencies)
...
// FIXED: New function that only moves backwards if deadline is NOT a working day
function getValidEndDate(date) {
    let d = new Date(date);
    // Only move backwards if the deadline itself is NOT a working day
    if (isWeekend(d) || isExcludedHoliday(d)) {
        // If deadline is weekend/holiday, find the last working day before it
        while (isWeekend(d) || isExcludedHoliday(d)) {
            d = addDays(d, -1);
        }
    }
    // If deadline is a working day, use it as-is
    return normalizeDate(d);
}

function findStartDate(endDate, duration) {
    // FIX: A 1-day task should start on endDate itself
